...
namespace Distribuidora.RF
{
    class Usuarios
    {
        ...
        public int validarUsuario(string nombre, string clave)
        {
            ...
            string consultaSQL =
//                "SELECT * FROM Users WHERE n_usuario = '" + nombre + "' AND password = '" + clave + "'";
            "SELECT * FROM Usuarios WHERE n_Usuario = '" + nombre + "' AND password = '" + clave + "'";
            Datos oBD = new Datos();
            tabla = oBD.consultar(consultaSQL);
 //           MessageBox.Show(ToString(tabla.Rows[0][1]));
